...
async def test_malicious_prompt(http_client, metrics_client, served_name):
    ...
    assert metrics_client.get_gauge(f'vllm_anomaly_last_rare_character{{model="{served_name}"}}') == 0
    assert metrics_client.get_gauge(f'vllm_anomaly_last_garbled{{model="{served_name}"}}') == 0
    assert metrics_client.get_gauge(f'vllm_anomaly_last_repetition{{model="{served_name}"}}') == 0
    assert metrics_client.get_gauge(f'vllm_anomaly_last_nan_value{{model="{served_name}"}}') == 0
