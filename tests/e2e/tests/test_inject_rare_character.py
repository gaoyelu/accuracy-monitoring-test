...
@pytest.mark.nightly
@pytest.mark.inject
async def test_inject_rare_character(vllm_service_b, http_client, metrics_client, served_name, injector, anomaly_data):
    ...
    before_detected = metrics_client.get_counter(f'vllm_anomaly_detected_total{{ill_type="1",model="{served_name}"}}')
